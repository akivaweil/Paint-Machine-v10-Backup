...
<head>
    ...
    <script>
        ...
        let isSystemPaused = false; // Client-side tracking of pause state
        ...
        function handleWebSocketMessage(message) {
            ...
            if (message.startsWith("STATE:")) {
                const stateName = message.substring(6);
                document.getElementById('machineStatusDisplay').textContent = "Machine State: " + stateName;
                updateButtonStates(stateName);
            } else if (message.startsWith("STATUS:PAUSED")) {
                isSystemPaused = true;
                const btnText = document.getElementById('pauseResumeBtnText');
                const btnIcon = document.getElementById('pauseResumeBtn').querySelector('.btn-icon');
                if (btnText) btnText.textContent = "Resume";
                if (btnIcon) btnIcon.textContent = "▶️";
                document.getElementById('machineStatusDisplay').textContent = "Machine State: PAUSED";
                // Consider disabling most buttons except resume
                updateButtonStates("PAUSED_STATE"); // Use a distinct state for button logic
                ...
                if (btnIcon) btnIcon.textContent = "⏸️";
                // The ESP will send its current state after resuming, so machineStatusDisplay will be updated by "STATE:" message
                // No need to send GET_STATUS if the server pushes state updates proactively.
            } else if (message.startsWith("CMD_ACK:") || message.startsWith("CMD_ERROR:") || message.startsWith("CMD_NOTE:") || message.startsWith("PRESSURE_POT_STATUS:")) {
                console.log("Server ACK/ERROR/NOTE: ", message);
                if (message.startsWith("PRESSURE_POT_STATUS:")) {
        ...
        function updateButtonStates(stateName) {
            ...
            const isPnP = stateName === 'PNP';
            const isPainting = stateName.startsWith('PAINTING') || stateName.startsWith('PAINT_SIDE'); // More robust check
            const isHoming = stateName === 'HOMING';
            const isCleaning = stateName === 'CLEANING';
            const isSystemActuallyPaused = stateName === 'PAUSED_STATE'; // Check for our special state

            // Master control for pause button based on client-side `isSystemPaused` and actual system state
            const pauseResumeButton = document.getElementById('pauseResumeBtn');
            if (pauseResumeButton) {
                if (isSystemPaused || isSystemActuallyPaused) { // If client thinks it's paused OR server says it's paused
                    pauseResumeButton.disabled = false; // Always allow resume
                    document.getElementById('pauseResumeBtnText').textContent = "Resume";
                    pauseResumeButton.querySelector('.btn-icon').textContent = "▶️";
                } else {
                    // Enable pause only during active, pausable states
                    pauseResumeButton.disabled = !(isPainting || isHoming || isCleaning || isPnP );
                    document.getElementById('pauseResumeBtnText').textContent = "Pause";
                    pauseResumeButton.querySelector('.btn-icon').textContent = "⏸️";
                }
            }

            // Disable other buttons if system is paused (by PAUSED_STATE) or in a non-interruptible state
            const systemEffectivelyPaused = isSystemPaused || isSystemActuallyPaused;

            document.getElementById('homeBtn').disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (paintSide1Btn) paintSide1Btn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (paintSide2Btn) paintSide2Btn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (paintSide3Btn) paintSide3Btn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (paintSide4Btn) paintSide4Btn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (paintAllSidesBtn) paintAllSidesBtn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (paintMultipleCoatsBtn) paintMultipleCoatsBtn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (cleanGunBtn) cleanGunBtn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (pnpButton) pnpButton.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (manualMoveToBtn) manualMoveToBtn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (manualRotateCWBtn) manualRotateCWBtn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (manualRotateCCWBtn) manualRotateCCWBtn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (setServoAngleSide1Btn) setServoAngleSide1Btn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (setServoAngleSide2Btn) setServoAngleSide2Btn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (setServoAngleSide3Btn) setServoAngleSide3Btn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (setServoAngleSide4Btn) setServoAngleSide4Btn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (saveAllServoAnglesBtn) saveAllServoAnglesBtn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (resetPaintSettingsBtn) resetPaintSettingsBtn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (loadPaintSettingsBtn) loadPaintSettingsBtn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (savePaintSettingsBtn) savePaintSettingsBtn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (updatePnpSettingsBtn) updatePnpSettingsBtn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            ...
            if (manualXInput) manualXInput.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (manualYInput) manualYInput.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (manualZInput) manualZInput.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (manualAngleInput) manualAngleInput.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (manualMoveBtn) manualMoveBtn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (manualRotateCWBtn) manualRotateCWBtn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
            if (manualRotateCCWBtn) manualRotateCCWBtn.disabled = systemEffectivelyPaused || (!isIdle && !isPnP);
        ...
        function sendCommand(command, value = null) {
            if (websocket && websocket.readyState === WebSocket.OPEN) {
                ...
                const commandObj = { command: fullCommand };
                websocket.send(JSON.stringify(commandObj));
                console.log("Sent command: ", JSON.stringify(commandObj));
            } else {
                console.log("WebSocket not connected. Command not sent: ", command);
                updateConnectionStatus(false, "Disconnected. Cannot send command.");
            }
        }
        ...
        <div class="integrated-main-card main-card">
            ...
            <div class="utility-buttons-grid">
                ...
                <button id="cleanGunBtn" class="main-btn blue" title="Clean Paint Gun" aria-label="Clean Gun" onclick="sendCommand('CLEAN_GUN')">
                    <span class="btn-icon">🧼</span> Clean Gun
                </button>
                <button id="pnpButton" class="main-btn mode" title="Switch to Pick and Place Mode" aria-label="Pick and Place Mode" onclick="sendCommand('ENTER_PICKPLACE')">
                    <span class="btn-icon">🛠️</span> PnP Mode
                </button>
                <button id="pauseResumeBtn" class="main-btn yellow" title="Pause or Resume Machine Operations" aria-label="Pause/Resume" onclick="togglePause()">
                    <span class="btn-icon">⏸️</span> <span id="pauseResumeBtnText">Pause</span>
                </button>
            </div>
        </div>
